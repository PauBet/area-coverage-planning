...
% Test time window
wstart = cspice_str2et('1998 MAR 29 12:30:00.000 TDB');
wend   = cspice_str2et('1998 MAR 29 13:30:00.000 TDB');
step   = 3*60;
et     = wstart:step:wend;
...
for i=1:length(et)
    ...
    [A, cv, fplist] = replanningSidewinder(et(i), ...
                wend + 4e3, tobs, inst, sc, target, roi, olapx, olapy, ...
                ax, 'b', []);
    clear grid2D planSidewinderTour;
    saveas(gcf, fullfile(pwd, 'test', ['rtest_', num2str(i), '.jpg']))
    delete(gcf)
end
