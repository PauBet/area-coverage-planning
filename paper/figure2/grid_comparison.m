    ...
    for j=1:size(grid, 2)
        if ~isempty(grid{i, j})
            ...
            h1 = plot(polyshape(fprinti.bvertices(:, 1), ...
                ...
                'EdgeColor', c2, 'LineWidth', 1.2, 'DisplayName', 'Ref. Tile');
            h2 = plot(lon, lat, '^', 'MarkerEdgeColor', [0.66,0.2,0.23], ...
                'MarkerSize', 5, 'DisplayName', 'Stare point', 'LineWidth', 1.2);
            hold on;
        end
    end
end
...
set(gca, 'XTick', xtick, 'YTick', ytick, 'XTickLabel', xtickstr, ...
    'YTickLabel', ytickstr, ...
    'Layer', 'top', 'GridColor', [0.1, 0.1, 0.1], 'GridAlpha', .3, 'GridLineStyle', ':', ...
...
for i=1:size(grid, 1)
    for j=1:size(grid, 2)
        if ~isempty(grid{i, j})
            lon = grid{i, j}(1); lat = grid{i, j}(2);
            fprinti = footprint(inittime, inst, sc, target, 'highres', lon, lat, 1);
            h1 = plot(polyshape(fprinti.bvertices(:, 1), ...
                fprinti.bvertices(:, 2)), 'FaceColor', c1, 'FaceAlpha', 0.2, ...
                'EdgeColor', c2, 'LineWidth', 1.2, 'DisplayName', 'Footprint');
            h2 = plot(lon, lat, '^', 'MarkerEdgeColor', [0.66,0.2,0.23], ...
                'MarkerSize', 5, 'DisplayName', 'Stare point', 'LineWidth', 1.2);
            hold on;
        end
