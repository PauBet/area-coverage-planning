...
% Check if the polygon defined by vertices [x, y] intercepts the
% anti-meridian line x = 180º...
aux = sort(x); % sort longitude values
ind = find(diff(aux) >= 180, 1); % find the discontinuity index
if isempty(ind) % no a.m. intercept
    xf = x;
    yf = y;
    return; 
end 
